...
class Mysql2PlantModel implements IModel<IPlant> {
  ...
  public async getAll(): Promise<IPlant[]> {
    const query = `SELECT id, breed, size, needs_sun as needsSun, 
      origin, water_Frequency as waterFrequency FROM plants`;
  ...
  public async create(plant: Omit<IPlant, 'id'>): Promise<IPlant> {
    ...
    const query = `INSERT INTO plants (breed, needs_sun, 
      origin, size, water_frequency) VALUES (?, ?, ?, ?, ?)`;
  ...
  public async getById(id: string): Promise<IPlant | null> {
    const query = `SELECT id, breed, size, needs_sun as needsSun, 
    origin, water_Frequency as waterFrequency FROM plants WHERE id = ?`;
    const values = [id];
  ...
  public async update(plant: IPlant): Promise<IPlant> {
    ...
    const query = `UPDATE plants SET breed = ?, needs_sun = ?, 
      origin = ?, size = ?, water_frequency = ? WHERE id = ?`;
